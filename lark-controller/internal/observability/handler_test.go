...
func TestMetricsExposeBoundedOperationalLabelsAndQueueAge(t *testing.T) {
	handler, err := observability.NewHandler("shadow", snapshotStore{snapshot: inbox.OperationalSnapshot{
		...
		EntitlementGrantResults: map[inbox.EntitlementGrantResultKey]int64{
			...
			{GrantType: "subscription_level", Status: "replayed"}: 1,
		},
		EntitlementGrantRetries:     map[string]int64{"principal_not_ready": 3},
		EntitlementGrantDeadLetters: map[string]int64{"integration_unauthorized": 1},
		PrincipalDisableJobStates:   map[string]int64{"held_shadow": 2, "unexpected": 1},
	...
	wantLines := []string{
		...
		`entitlement_grant_total{status="replayed",type="subscription_level"} 1`,
		`entitlement_grant_retry_total{reason="principal_not_ready"} 3`,
		`entitlement_dead_letter_total{reason="integration_unauthorized"} 1`,
		`lark_principal_disable_jobs{state="held_shadow"} 2`,
		`lark_principal_disable_jobs{state="other"} 1`,
		`principal_disable_total{result="applied"} 1`,
