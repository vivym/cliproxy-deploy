...
1. backup runner 先原子取得 host-only `maintenance.session`，再取得 `maintenance.lock/mode=backup`。session 排斥 restore/correction/config writer；lock 存在时常驻 New API/Controller entrypoint 一律拒绝启动，临时 mutation endpoint 也因 mode 不匹配拒绝写入。
2. runner 记录原运行集合，按 Traefik、Controller、New API config endpoint、New API、Sub2API 顺序停止入口和全部应用 writer。已经停止的服务保持停止；Controller webhook 连接失败由 Lark 重试。此后没有 Controller -> New API 在途命令、Postgres producer 或 Redis writer。
...
4. barrier 保持期间，先拒绝 runtime source 中任何 symlink 或 hard-linked regular file，再依次生成两个 Postgres custom dump、两个 Redis 持久化副本和 deployment runtime/policy 归档。任一步失败都不发布最终包，成功包不会包含 restore 拒绝的 link entry。
...
- [x] 从当前提交发布并固定 New API fork、Controller、correction CLI 和 config CLI image digest；当前周额度 revisions 已取得 registry index、逐平台 provenance/SBOM、匿名拉取和 amd64 入口收据。
...
`9a6d825db04ef28aa81bea14671c0ddb778eac39`。当前 immutable reference、两架构 digest、
attestation、runtime 合同、amd64 执行结果和临时产物清理记录以
[Compose 灰度运行手册](../runbooks/lark-controller-compose-rollout.md#当前周额度候选的-registry-收据)
为唯一收据源。该收据同时保留本地 OCI 与公开 registry 的证据边界；镜像发布门禁已通过，
但不替代真实 tenant、生产网络或恢复演练门禁。
...
转换 media type 都可能产生不同 digest。2026-08-25 的周额度实现前 GHCR registry receipt 如下：
...
这些 registry 收据不能用于 New API `a624396d` 或 deployment `9a6d825` 的周额度候选。
