...
services:
  ...
  cliproxyapi:
    ...
    labels:
      ...
      - "traefik.http.routers.cliproxyapi-api.rule=Host(`${API_HOST}`)"
      ...
      - "traefik.http.routers.cliproxyapi-api-management.rule=Host(`${API_HOST}`) && (PathPrefix(`/management`) || PathPrefix(`/v0/management`))"
      ...
      # Admin hostname.
      - "traefik.http.routers.cliproxyapi-admin.rule=Host(`${ADMIN_HOST}`)"
      ...
      - "traefik.http.middlewares.admin-auth.basicauth.users=${TRAEFIK_BASIC_AUTH_USERS}"
